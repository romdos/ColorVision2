...
void CViewImageVw::OnDraw(CDC* pDC)
{
	...
	if((m_Video_ImageSegmented)&&(pApp->m_GrayScaleOpened))
	{
	...
	StripWidth1 = CurrentStrip[coun_strip1].StripWidth/ratio_im;
	StripWidthPrev1 = CurrentStrip[coun_strip1].StripWidthPrev/ratio_im;
	...
	if((old_col_num<24)&&(old_col_num<old_col_number))
	{
	bunch_average_hue=
	pApp->ColorImageProcess->ColorInt[coun_strip1].ColoredIntervalsStructure->AverageHue[old_col_num];
	bunch_average_sat=
	pApp->ColorImageProcess->ColorInt[coun_strip1].ColoredIntervalsStructure->AverageSat[old_col_num];
	bunch_average_gray=
	pApp->ColorImageProcess->ColorInt[coun_strip1].ColoredIntervalsStructure->AverageGray[old_col_num];
	Beg_bunch=
	pApp->ColorImageProcess->ColorInt[coun_strip1].ColoredIntervalsStructure->BegInterv[old_col_num]/ratio_im;
	End_bunch=
	pApp->ColorImageProcess->ColorInt[coun_strip1].ColoredIntervalsStructure->EndInterv[old_col_num]/ratio_im;
	...
	PeakColor5=RGB(Scoor1,Scoor2, Scoor3);
	PeakColor6=RGB(NScoor1,NScoor2, NScoor3);
	pHuePen2=new CPen(PS_SOLID, 3, PeakColor6);

	PtrOldPen = pDC->SelectObject(pHuePen2);
	HorizVert=pApp->HorizontalVertical;
	if(HorizVert)
	{
	OrPoint.y = (WMHeight/ratio_im - Beg_bunch);

	OrPoint.x= StripWidthPrev1*coun_strip1 + StripWidth1/2;
	pDC->MoveTo(OrPoint);
	EndPoint.x = OrPoint.x;
	EndPoint.y = WMHeight/ratio_im - End_bunch;
	pDC->LineTo(EndPoint);
	}
	else
	{
...
CViewImageVw::CameraInputDraw(CDC* pDC)
{
	...
	0,                          // SrcY
	0,                          // nStartScan
	(WORD)pm_Bitmap->bmHeight,    // nNumScans
	...
	int* scoor1, int* scoor2, int* scoor3, int* nscoor1, int* nscoor2, int* nscoor3)
{
	...
	if (side_triangle == 0)
	{
		...
		GrayLev = int(0.3*((float)Scoor1) + 0.59*((float)Scoor2) + 0.11*((float)Scoor3));
		GrayLev = GrayLev / 4;
		if ((GrayLev>BunAverGr) && (BunAverGr<40))
		{
